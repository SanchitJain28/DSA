...
function RecursiveFactorialTheOriginal(number: number) {
...
function RecursiveFactorialTheChatGpt(number: number): number {
  if (number === 1) return 1;
  return number * RecursiveFactorialTheChatGpt(number - 1);
}

function NonRecursiveFactorial(number: number) {
  let result = 1;
...
function normalFibonnaciSequence(limit: number) {
  let array = [0, 1];
...
function RecursiveFibonacciSequence(number: number): number {
