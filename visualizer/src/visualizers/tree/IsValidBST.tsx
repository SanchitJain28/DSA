import { useMemo, useState } from "react";
import TreeVisualizerLayout from "../../components/layout/TreeVisualizerLayout";
import TestCaseSwitcher, {
  type TestCase,
} from "../../components/shared/TestCaseSwitcher";
import { computeLayout } from "../../core/tree/layout";
import { generateFrames } from "../../core/tree/frames/isValidBSTFrames";
import { isValidBSTCode } from "../../core/tree/sourcecode/isValidBST";
import { TreeNode } from "../../core/tree/TreeNode";

type TreeTestCase = TestCase<TreeNode | null>;

const TEST_CASES: TreeTestCase[] = [
  {
    id: "tc1",
    name: "Valid BST",
    data: (() => {
      const root = new TreeNode(2, "2");
      root.left = new TreeNode(1, "1");
      root.right = new TreeNode(3, "3");
      return root;
    })(),
  },
  {
    id: "tc2",
    name: "Invalid BST",
    data: (() => {
      const root = new TreeNode(5, "5");
      root.left = new TreeNode(1, "1");
      root.right = new TreeNode(4, "4");
      root.right.left = new TreeNode(3, "3");
      root.right.right = new TreeNode(6, "6");
      return root;
    })(),
  },
  {
    id: "tc3",
    name: "Invalid (Out of Bounds)",
    data: (() => {
      const root = new TreeNode(5, "5");
      root.left = new TreeNode(4, "4");
      root.right = new TreeNode(6, "6");
      root.right.left = new TreeNode(3, "3"); // 3 < 5!
      root.right.right = new TreeNode(7, "7");
      return root;
    })(),
  },
];

export default function IsValidBST() {
  const [testCaseIdx, setTestCaseIdx] = useState(0);

  const rootAndFrames = useMemo(() => {
    const root = TEST_CASES[testCaseIdx].data;
    return { root, frames: generateFrames(root || null) };
  }, [testCaseIdx]);

  const layout = useMemo(() => {
    return rootAndFrames.root
      ? computeLayout(rootAndFrames.root)
      : { nodes: [], edges: [] };
  }, [rootAndFrames.root]);

  return (
    <TreeVisualizerLayout
      title="Validate Binary Search Tree"
      theme="indigo"
      layout={layout}
      frames={rootAndFrames.frames}
      code={isValidBSTCode}
      headerChildren={
        <TestCaseSwitcher
          testCases={TEST_CASES}
          currentIndex={testCaseIdx}
          onChange={setTestCaseIdx}
        />
      }
    />
  );
}
