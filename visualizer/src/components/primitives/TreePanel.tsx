...
import type {
  ...
  TreeLayoutNode,
  TreeLayoutEdge,
} from "../../core/structures/tree/types";

export type { TreeState, TreeLayoutNode, TreeLayoutEdge };
...
export default function TreePanel({ state }: TreePanelProps) {
  ...
  const { totalWidth, totalHeight, originX, originY } = useMemo(() => {
    ...
    return {
      totalWidth: Math.max(360, maxX - minX + paddingX * 2),
      totalHeight: Math.max(220, maxY - minY + paddingY * 2),
      ...
      <div className="absolute inset-0">
        <AnimatePresence mode="popLayout">
          {nodes.map((node) => {
            ...
            if (node.isNull) {
              return (
                <motion.div
                  ...
                  className={`absolute w-4 h-4 -ml-2 -mt-2 rounded-full border flex items-center justify-center shadow-md ${
                    isActive ? "z-20 ring-1 ring-[#b08a8a]/50" : "z-0"
                  }`}
                  style={{ left: posX, top: posY }}
                >
                  {isActive && (
                    <span className="text-[8.5px] font-bold text-[#b08a8a]">
                      ∅
                    </span>
                  )}
                </motion.div>
            ...
            const isTarget =
              node.id === state?.targetNodeId ||
              state?.targetNodeIds?.includes(node.id) ||
              node.status === "target";
            ...
            let border = "#34343c";
            let shadow =
              "0 4px 10px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.08)";
            ...
            return (
              <motion.div
                ...
                className={`absolute w-10 h-10 -ml-5 -mt-5 rounded-full border-[1.5px] flex items-center justify-center font-['JetBrains_Mono',monospace] font-bold text-[12.5px] select-none z-10 ${textColor}`}
                style={{
                  left: posX,
                  top: posY,
                  background: bg,
